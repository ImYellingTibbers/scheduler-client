...
export default function SchedulePage() {
  ...
  const holidaysByDate = useMemo(() => {
    const m = new Map();
    for (const h of state.holidays) m.set(h.date, h.localName || h.name || 'Holiday');
    return m;
  }, [state.holidays]);

  const coverageByDateSite = useMemo(() => {
    const m = new Map();
    for (const c of state.coverage) m.set(`${c.date}|${c.siteId}`, Number(c.requiredCount) || 0);
    return m;
  }, [state.coverage]);
  ...
  const shiftsForSelected = useMemo(() => {
    ...
    return state.shifts
      ...
      .map((sh) => ({
        ...sh,
        userName: state.users.find((u) => u._id === sh.userId)?.name || 'Unknown',
        siteName: state.sites.find((s) => s._id === sh.siteId)?.name || sh.siteId,
      }));
  ...
  return (
    <section className="page page--schedule">
      <div className="toolbar">
        <button onClick={() => setMonthDate(new Date(monthDate.getFullYear(), monthDate.getMonth()-1, 1))} aria-label="Previous month">‹</button>
        <h2>
          {monthDate.toLocaleString('default', { month: 'long' })} {monthDate.getFullYear()}
        </h2>
        <button onClick={() => setMonthDate(new Date(monthDate.getFullYear(), monthDate.getMonth()+1, 1))} aria-label="Next month">›</button>
      </div>
